...
void setup() {
  ...
  lH = hour();
  lMil = millis();
  //tiTable = loadTable("dewey.csv", "header");
  tiTable = loadTable("classification.csv", "header");
  dewTable = loadTable("dewey.csv", "header");
  rogTable = loadTable("roget.csv", "header");
  ...
  btnUseDewey = new Button(770, 320, 85, 20, "Use Dewey", #DEDCB5);
  btnUseRoget = new Button(865, 320, 95, 20, "Use Roget", #DEDCB5);
  btnUseBoth = new Button(660, 320, 100, 20, "Use Both", #DEDCB5);
...
void drawUI() {
  ...
  fill(100);
  textSize(11);
  text("© Ken Kapptie | 2018 | Data from Dewey Decimal & Roget's Thesaurus - roget.org.", 40, 393);
...
void getNewRows() {
  if (dbselect == 'b') {
    ...
    row1 = int(random(2, dewTable.getRowCount()));
    row2 = int(random(2, dewTable.getRowCount()));
  } else if (dbselect == 'r') {
    row1 = int(random(2, rogTable.getRowCount()));
    row2 = int(random(2, rogTable.getRowCount()));
  }
}
...
void mouseReleased() {
  if (btnUseDewey.hov) {
    dbselect = 'd';
    getNewRows();
  } else if (btnUseRoget.hov) {
    dbselect = 'r';
    getNewRows();
  } else if (btnUseBoth.hov) {
    dbselect = 'b';
    getNewRows();
  }
